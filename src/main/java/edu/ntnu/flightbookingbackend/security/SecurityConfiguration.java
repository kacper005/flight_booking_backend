 ...
 * The @EnableMethodSecurity is needed so that each endpoint can specify wich role it requires.
 */
@Configuration
  ...
  public SecurityFilterChain configure(HttpSecurity http) throws Exception {
    http.csrf(AbstractHttpConfigurer::disable)
        .authorizeHttpRequests(auth -> auth
            ...
            .requestMatchers("/flights/**", "/airports/**").permitAll()

            // Feedback: Only logged in users kan create and update feedback
            .requestMatchers(HttpMethod.POST, "/feedback/**").hasAuthority("USER")
            .requestMatchers(HttpMethod.PUT, "/feedback/**").hasAuthority("USER")
            .requestMatchers(HttpMethod.DELETE, "/feedback/**").hasAuthority("USER")

            // Booking: Only logged in users can book flights
            .requestMatchers("/booking/**").hasAuthority("USER")

            // Flight-administration: Only ADMIN can add, delete and update flights and airports
